...
const realizarPuja = async (req, res) => {
    ...
    try {
        ...
        if (subastas.length === 0) {
            await connection.rollback();
            return res.status(400).json({ mensaje: 'Subasta no disponible o finalizada' });
        }
        ...
        if (monto <= subasta.oferta_actual) {
            ...
            return res.status(400).json({ 
                mensaje: `La puja debe ser mayor a ${subasta.oferta_actual}` 
            });
        }
...
misPujas = async (req, res) => {
    try {
        const usuarioId = req.usuarioId;
        const [pujas] = await db.promise().query(`
            SELECT o.*, s.nombre as subasta_nombre, s.activa, s.fecha_fin,

...
const SubastaDetallePage = () => {
    ...
    return (
        <div className="subasta-detalle">
            <button className="btn-volver" onClick={() => navigate('/subastas')}>
                ← Volver
            </button>
            ...
            {subasta.foto_url && (
                <img src={subasta.foto_url} alt={subasta.producto_nombre} className="detalle-imagen" />
            )}

            <div className="detalle-info">
                <p className="descripcion">{subasta.producto_descripcion}</p>
                <p><strong>Vendedor:</strong> {subasta.vendedor_nombre}</p>
                <p><strong>Precio inicial:</strong> ${subasta.precio_inicial}</p>
                <p><strong>Precio mínimo:</strong> ${subasta.precio_minimo}</p>
                <p><strong>Oferta actual:</strong> ${subasta.oferta_actual}</p>
                <p><strong>Tiempo restante:</strong> {tiempoRestante}</p>
            </div>

            {puedePujar && (
                <button className="btn-pujar" onClick={() => setMostrarModal(true)}>
                    Realizar Puja
                </button>
            )}

            {esMiSubasta && subasta.activa && (
                <div className="tu-subasta-info">
                    <p>Esta es tu subasta. No puedes pujar en ella.</p>
                </div>
            )}
